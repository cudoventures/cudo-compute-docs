---
title: JupyterLab
description: Jupyter Notebooks on CUDO Compute.
---
...
## Get started
Go to the apps section in the CUDO Compute console and click either the small, medium or large instance of JupyterLab. This will give you some good default settings but you can fully customise your deployment at the next step.
...
## Using Jupyterlab
When you deploy the VM you will be shown the VM information page. On the left hand side there is a pane called 'Metadata'.
...
You will need to use the VM IP address from the VM information page and the ``port`` from the meta data to connect to your JupyterLab.
...
- Using a tool like ``scp`` to copy files from another cloud machine or your local machine, you will need to have SSH keys set up on your VM. Any files copied to the VMs ``/cudo`` directory will appear in the work directory in JupyterLab
