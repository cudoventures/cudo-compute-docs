...
{/* <Checklist>
  <CheckItem>Provision and manage <Link href="/guides/clusters/0.index">clusters</Link> (GPU & CPU capacity)</CheckItem>
  <CheckItem>Create, start, stop and resize <Link href="/guides/virtual-machines/0.index">virtual machines</Link> & bare metal machines</CheckItem>
  <CheckItem>Attach, detach, and resize <Link href="/guides/volumes/0.index">volumes</Link> and manage <Link href="/guides/disks/0.index">disks</Link></CheckItem>
  <CheckItem>Manage <Link href="/guides/networks/0.index">networks</Link>, security groups, and ingress controls</CheckItem>
  <CheckItem>Import, list and manage machine <Link href="/guides/images/0.index">images</Link></CheckItem>
  <CheckItem>Work with <Link href="/guides/object-storage/0.index">object storage</Link></CheckItem>
...
<CardGroup cols={3}>
  ...
  <Card title="Errors" icon="warning" href="/api/errors">
...
The `v1` segment denotes the stable API surface. Additive, backwards-compatible changes (new fields, endpoints) may occur without prior notice. Any breaking changes will be announced in the <Link href="/api/changelog">Changelog</Link> with advanced notice.
...
See the dedicated <Link href="/api/authentication">Authentication</Link> page for creating and rotating keys, best practices, and troubleshooting. Requests without a valid key return `401 UNAUTHENTICATED`.
...
<CardGroup cols={3}>
  <Card title="cudoctl CLI" icon="terminal" href="/tools/cli">Script resource lifecycle operations quickly.</Card>
  <Card title="Python Client" icon="python" href="/tools/python-client">Programmatic convenience wrapper.</Card>
...
Additional automation tooling examples (dstack, Terraform, Python) are covered in the <Link href="/tools/cli">Tools</Link> section.
...
<AccordionGroup>
  <Accordion title="Get an API key" icon="key" defaultOpen>
    Visit the <Link href="/api/authentication">Authentication</Link> guide to create your first key.
  </Accordion>
  ...
  <Accordion title="Handle errors gracefully" icon="warning">
    Review <Link href="/api/errors">Errors</Link> and implement structured handling + retries.
  </Accordion>
  <Accordion title="Track changes" icon="history">
    Monitor new endpoints & updates in the <Link href="/api/changelog">Changelog</Link>.
  </Accordion>
</AccordionGroup>
